...
| データ | 同じ `profiles.json` のパス、PascalCase の 6 項目、JSON 配列、V1 が読める日時/数値形式を維持 |
...
- ベータはインストーラー付きの手動配布。GitHub Release の `prerelease: true` / `make_latest: false` を明示し、正式版扱いにしない。タグ名だけに依存しない。
